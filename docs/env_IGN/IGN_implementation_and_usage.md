...
本モジュールは，FlightPinの挿入状態（`Fp=1`）を観測した後，抜去状態（`Fp=0`）で気圧高度が設定高度を連続30回超えると警告シーケンスを開始し，その後モデルロケットのイグナイタへ通電する．高度条件の設定・判定方法は[気圧高度によるIGN開始条件](IGN_altitude_trigger.md)を参照．
