...
本モジュールは，FlightPinの挿入状態（`Fp=1`）を観測した後，抜去状態（`Fp=0`）で「気圧高度が設定高度以上を連続30回」または「気圧高度が60秒以上更新されず，LOW確認から1時間以上経過し，MissionBusのGPS高度が10km以上（UT更新・受信鮮度確認付き）」を満たすと警告シーケンスを開始し，その後モデルロケットのイグナイタへ通電する．MissionBusの変更は不要．時間の設定・GPS判定の限界と最新のシーケンス時間は[IGN開始条件](IGN_altitude_trigger.md)を参照．
...
電力テレメトリ`I`は待機中は1 Hz，点火シーケンス開始後は10 Hzで送信し，Doneへ移行すると測定・送信を1 Hzへ戻す．シーケンス状態テレメトリ`S`は状態が変化した時点および1秒周期で送信する．
...
6. シーケンス中にFlightPinを再挿入し，HIGH・LOWが直ちにLOWとなって`Ph`が5へ戻ることを確認する．
7. 以上を確認したのち，実イグナイタを接続した試験へ移る．
...
開始要求が受理されるとGPIO 21の立ち上がり割り込みをアームする．シーケンス中にFlightPinが再挿入されると，ISR対応の`abortSequenceFromISR()`がLOW側，HIGH側の順に出力を直ちに停止し，中止要求をラッチする．状態機械のDisarmedへの遷移，ログおよびテレメトリ送信は通常のIGNタスクで行う．開始要求前およびDone，Fault，Disarmedへ移行した後はこの割り込みによる中止を行わない．
...
- シーケンス中のFlightPin再挿入はGPIO割り込みで検出し，点火出力を直ちに停止する．
