...
> 一番手軽なのは、`マイ早稲田 -> ログイン -> 個人情報照会・変更 -> 学生基本情報`にある画面のスクショですね。

![student-portal](/docs/image/github-copilot/student-portal.png)
...
- ここで学生基本情報のスクショが学生証の画像をアップロードをしてください。
- 学生基本情報のスクショの場合は左上の`Select...`を選択して`8`のStudent Potalなどと書いてるものを選択してください。
- 学生証の場合は`1`にStudent ID cardの選択肢があるかと思います。
