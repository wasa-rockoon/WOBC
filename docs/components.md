...
IGNモジュール側は `begin(false)` で初期化のみを行い，FlightPinの抜去検出時に`startSequence()`を呼ぶ．起動時から`Fp`が0の場合，値が欠損している場合，整数以外または0・1以外の場合は開始しない．`abortSequence()` は内部エラー時に点火出力を直ちに停止する．同一boot中に開始できるのは1回だけとする．
