...
コンポーネントIDは `36` (0x24)．モジュール内の初期化成功後はDisarmedで待機し，FlightPinの`Fp`で挿入状態（1）を観測した後，抜去状態（0）へ変化するとフェーズ0を開始する．開始後はBBMシーケンスを実行し，タスクの遅延時は警告時間を省略せず，点火時刻を安全側（遅い側）へ延長する．

IGNモジュール側は `begin(false)` で初期化のみを行い，FlightPinの抜去検出時に`startSequence()`を呼ぶ．起動時から`Fp`が0の場合，値が欠損している場合，整数以外または0・1以外の場合は開始しない．シーケンス開始後はFlightPinの挿入をGPIOの立ち上がり割り込みで監視し，Startup，Countdown，FinalまたはIgnition中に再挿入された場合は点火出力を直ちに停止してDisarmedへ移行する．同一boot中に開始できるのは1回だけとする．
...
テレメトリIDは`F`．フライトピンは1が挿入，0が抜去を表す．

| エントリ名 | データ型 | 内容 |
| --- | --- | --- |
|Fp|int|フライトピン状態（0: 抜去，1: 挿入）|
|ET|int|現在の状態になってからの経過時間(ms)|
|TS|int|タイムスタンプ(ms)|
