...
constexpr float Kp_roll  = 1.3f;   // ロール比例ゲイン
constexpr float Kd_roll  = 0.35f;  // ロール微分ゲイン
...
constexpr uint32_t CONTROL_DELAY_MS = 1500;          // 発射検知→制御開始までの遅延 [ms]（ランチャー離脱待ち）
...
protected:
  ...
  // --- サーボ PD制御の内部状態 ---
  struct {
    PIDController roll{0.0f};      // ロール軸 PD制御器（prev_error 保持）
  ...
  // 相補フィルターのメインループ: ジャイロ積分 → 加速度補正 → 正規化。
