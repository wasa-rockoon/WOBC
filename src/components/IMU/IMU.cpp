...
IMU9::IMU9(TwoWire& wire, uint8_t unit_id, unsigned sample_freq_hz, int mode)
...
void IMU9::setup() {
  ...
  if (mode == IMU_DATA_WITH_MADGWICK_6) {
    gyro_offset_ = calibrate_gyro();
    filter.begin(freq_);
  }
  if (mode == IMU_DATA_WITH_KALMAN_6) {
    gyro_offset_ = calibrate_gyro();
...
void IMU9::SampleTimer::callback() {
  ...
  if (IMU9_.mode == IMU_DATA_WITH_MADGWICK_6) {
