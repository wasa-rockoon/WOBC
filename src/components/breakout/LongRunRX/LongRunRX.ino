...
void loop() {
  ...
  if (len > 0) {
    ...
    Serial.printf("message1 (%d byte, %d dB):", len, lora1.getRSSI());
    
    for (unsigned i = 0; i < len; i++) {
      Serial.print((char)rx1[i]);  // ASCII文字として表示
    }
    Serial.println();
  ...
  if (len > 0) {
    ...
    Serial.printf("message2 (%d byte, %d dB):", len, lora2.getRSSI());
    
    for (unsigned i = 0; i < len; i++) {
      Serial.print((char)rx2[i]);  // ASCII文字として表示
    }
    Serial.println();
